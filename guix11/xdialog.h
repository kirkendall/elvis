...
typedef enum {
	ELVFT_DEFAULT,	/* dummy value, for fields with no explicit type */
	ELVFT_ONEOF,	/* one of a preset list; includes boolean */
	ELVFT_NUMBER,	/* numeric field */
	ELVFT_STRING,	/* string field */
	ELVFT_FILE,	/* string field where <Tab> does filename completion */
	ELVFT_LOCKED	/* non-editable field */
} X_FIELDTYPE;
typedef struct
{
